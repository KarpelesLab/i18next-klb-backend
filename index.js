...
class Backend {
  ...
  read(language, namespace, callback) {
    if (language.length != 5) {
      ...
      // this can happen if i18next attempts to load special language "dev" or "en" instead of "en-US"
      callback(null, {});
      return;
    }
    ...
    let pfx = "";
    if (typeof FW !== "undefined" && typeof FW.prefix !== "undefined") {
        pfx = FW.prefix;
    }
    // pfx=""
    // pfx="/l/en-US"
    // pfx="/b/test/l/en-US/z/foobar"
    let newpfx = pfx.replace(/\/l\/[a-z]{2}-[A-Z]{2}/, "/l/"+language);
    if (newpfx == pfx) {
        newpfx = "/l/"+language;
    }
